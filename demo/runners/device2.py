...
async def main(args):
    device2_agent = await create_agent_with_args(args, ident="device2")

    try:
        log_status(
            "#1 Provision an agent and wallet, get back configuration details"
            + (
                f" (Wallet type: {device2_agent.wallet_type})"
                if device2_agent.wallet_type
                else ""
            )
        )
        agent = Device2Agent(
            "device2.agent",
            device2_agent.start_port,
            device2_agent.start_port + 1,
            genesis_data=device2_agent.genesis_txns,
            no_auto=device2_agent.no_auto,
            tails_server_base_url=device2_agent.tails_server_base_url,
            timing=device2_agent.show_timing,
            multitenant=device2_agent.multitenant,
            mediation=device2_agent.mediation,
            wallet_type=device2_agent.wallet_type,
            seed=device2_agent.seed,
        )

        device2_agent.public_did = True
        # TODO: Create schema
        device2_schema_name = "employee id schema"
        device2_schema_attrs = ["employee_id", "name", "date", "position"]
        await device2_agent.initialize(
            the_agent=agent,
            schema_name=device2_schema_name,
            schema_attrs=device2_schema_attrs,
        )
        ...
        # await device2_agent.generate_invitation(display_qr=True, wait=True)

        # log_status("Input new invitation details")
        # await input_invitation(device2_agent)
        ...
        if device2_agent.show_timing:
            timing = await device2_agent.agent.fetch_timing()
            if timing:
                for line in device2_agent.agent.format_timing(timing):
                    log_msg(line)

    finally:
        terminated = await device2_agent.terminate()
...
if __name__ == "__main__":
    parser = arg_parser(ident="device2", port=8050)
